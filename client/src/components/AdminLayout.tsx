...
export default function AdminLayout({ children, title }: AdminLayoutProps) {
  ...
  const roleLabel: Record<string, string> = { platform_admin: "Administrador", platform_analyst: "Analista RH", platform_auditor: "Auditor" };
        ...
        <div className={cn("flex items-center border-b border-border h-16 px-4 gap-3", collapsed && "justify-center px-0")}>
          {!collapsed ? (
            <div className="flex flex-col leading-tight">
              <span className="text-sm font-bold">Smart<span className="text-primary">Doc</span>Plan</span>
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Plataforma Admin</span>
            </div>
          ) : (
            <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center"><span className="text-primary-foreground text-xs font-bold">S</span></div>
          )}
        </div>
        <NavList />
